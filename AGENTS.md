...
| `heading-h3` ainda não existe no Figma DS | `packages/ui/src/tokens/typography.ts` | Token promovido no código (aprovação TL, #304) para o título de "Meu Perfil" (24px, escala pulava de h1/48px para h2/36px direto para body/label). `lineHeight: 1.3` extrapolado da progressão h1(1.2)/h2(1.25) — falta o designer confirmar o valor e criar a variável na coleção Typography do Figma DS |
