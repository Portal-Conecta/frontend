...
describe("findActiveExecutionClient", () => {
  it("ignora execuções de outro slot quando a API devolve resultados sem filtrar", async () => {
    ...
    const wrongType = {
      ...execution,
      id: "wrong-type",
      checklistType: "POST_BREAK" as const,
    ...
    await expect(
      findActiveExecutionClient("c1", "r1", "ARRIVAL"),
    ).resolves.toEqual(matching);
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "/api/checklist/executions?classId=c1&roomId=r1&checklistType=ARRIVAL&size=50",
    );
  });
});
...
describe("saveDraftAnswersClient", () => {
  it("faz PATCH em /{id}/draft com as respostas parciais", async () => {
    ...
    await expect(saveDraftAnswersClient("e1", { answers })).resolves.toEqual(
      execution,
    );
  ...
  it("mapeia 409 (execução travada por issue não validada) para HttpError", async () => {
    ...
    await expect(
      saveDraftAnswersClient("e1", { answers: [] }),
...
describe("submitExecutionClient", () => {
  it("faz POST em /{id}/submit com as respostas", async () => {
    ...
    const answers = [{ itemKey: "oleo", value: "COMPLIANT" as const }];
    await expect(
      submitExecutionClient("e1", { answers }),
...
describe("updateExecutionAnswersClient", () => {
  it("faz PATCH em /{id}/answers com as respostas", async () => {
    ...
    await expect(
      updateExecutionAnswersClient("e1", { answers }),
    ).resolves.toEqual(execution);
