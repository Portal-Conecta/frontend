...
const withAppliedLog: NonNullable<Story['render']> = (args) => {
  const [applied, setApplied] = useState<string>('(nenhum filtro aplicado ainda)')
  return (
    <div className="flex flex-col gap-4">
      <ChecklistFilters {...args} onApply={(filters) => setApplied(JSON.stringify(filters, null, 2))} />
      <pre className="rounded-md bg-background-default p-4 text-label-xs">{applied}</pre>
    </div>
  )
}
