...
export function defaultDashboardPeriod(days = 30): {
  from: string;
  to: string;
} {
  const to = new Date();
...
export type PeriodValidation = { ok: true } | { ok: false; message: string };
...
export function validateDashboardPeriod(
  ...
  if (from > to) {
    return {
      ok: false,
      message: "A data 'De' não pode ser posterior a 'Até'.",
    };
  }
  ...
  if (to > today) {
    return {
      ok: false,
      message: "O fim do intervalo não pode estar no futuro.",
    };
  }
  return { ok: true };
}
