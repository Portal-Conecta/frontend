...
export function findStatsValue(entries: StatsEntry[], predicates: string[]): number {
 ...
 */
export function parseTaxaConclusao(entries: StatsEntry[] | undefined | null): TaxaConclusaoParsed {
  const list = entries ?? []
  ...
  if (total <= 0 && list.length > 0 && !list.some((e) => /rate|total|submit/i.test(e.label))) {
